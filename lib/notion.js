...
// Возвращает { changed: true/false } — false означает, что статус и так уже
// был нужным (повторное нажатие, сетевой ретрай и т.п.), запись в Notion не
// потребовалась. Это используется, чтобы не слать уведомление повторно.
async function setRsvpStatus(eventId, user, status) {
  const existing = await findRsvpPage(eventId, user.id);

  if (existing) {
    const currentStatus = existing.properties['Статус'] && existing.properties['Статус'].select
      ? existing.properties['Статус'].select.name
      : null;

    // Статус уже такой, каким его пытаются сделать — реального изменения нет.
  ...
  if (status !== 'активно') return { changed: false };
